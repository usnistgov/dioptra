...
def create_model(
    ...
    model = KERAS_CLASSIFIERS_REGISTRY[model_name](
        input_shape=dataset.meta.image_shape,
        classes=dataset.meta.num_classes,
        **model_options,
    )
...
def train_model(
    ...
    model.fit(
        train_dataset.data,
        validation_data=val_dataset.data if val_dataset else None,
        epochs=epochs,
        verbose=2,
        callbacks=callbacks,
        **fit_options,
...
def evaluate_predictions(
    ...
    labels = np.concatenate([y for _, y in dataset.data])
...
def model_predict(
    ...
    predictions = model.predict(
        dataset.data, callbacks=[DioptraMetricsLoggingCallback(LOGGER)], verbose=0
    )
