...
from .data import Dataset, create_transformed_dataset
...
def carlini_wagner(
    ...
    attack = method(
        classifier=KerasClassifier(model, clip_values=dataset.meta.value_range),
...
def create_adversarial_patch(
    ...
    y_target = ops.repeat(
        [ops.one_hot(target, dataset.meta.num_classes)], dataset.meta.batch_size, axis=0
    )

    for x, _ in dataset.data.take(max_iter):
