...
class DatasetMetadata:
    """
    Metadata for an annotated AI/ML image dataset.
    """
...
class Dataset:
    """
    A dataclass to store a dataset and its associated metadata.
    """
...
def _load_dataset_from_builder(
    ...
    Args:
        ...
        normalize: If true, the data is normalized from [0, 255] to [0, 1].
        image_size: Size to
    ...
    def normalize_data(x, y):
        ...
        normalize_val = 255.0 if normalize else 1.0
    ...
    for name, ds in dataset.items():
        ...
        if normalize is not None:
            ds = ds.map(normalize_data, num_parallel_calls=tf.data.AUTOTUNE)
    ...
    metadata = DatasetMetadata(
        ...
        value_range=(0.0, 1.0) if normalize else (0.0, 255.0),
        batch_size=batch_size,
        num_classes=num_classes,
    )

    train = dataset.get("train", None)
    val = dataset.get("val", None)
    test = dataset.get("test", None)

    return (
        Dataset(data=train, meta=metadata) if train else None,
        Dataset(data=val, meta=metadata) if val else None,
        Dataset(data=test, meta=metadata) if test else None,
    )
...
def create_transformed_dataset(
    ...
    data = dataset.data
    data = data.map(fn, num_parallel_calls=tf.data.AUTOTUNE, deterministic=True)
    data = data.map(set_shape, num_parallel_calls=tf.data.AUTOTUNE, deterministic=True)

    if save_dataset:
        options = tf.data.Options().merge(data.options())
        options.dataset_name = str(uuid.uuid4())

        data.save(options.dataset_name)
        with open(Path(options.dataset_name) / "metadata.json", "w") as f:
            json.dump(asdict(dataset.meta), f)

        # reload from disk to prevent repeat computation
        data = tf.data.Dataset.load(options.dataset_name).with_options(options)

    return Dataset(data=data, meta=dataset.meta)
