...
from .artifacts_restapi import upload_artifact_to_restapi, get_artifacts_for_job
...
@pyplugs.register
def download_all_artifacts_for_job(
    job_id: str, artifact_path: str, destination_path: Optional[str] = None
) -> str:
    ...
    """
    uris = get_artifacts_for_job(job_id)
    for uri in uris:
        if uri.endswith(artifact_path):
            download_path: str = mlflow.artifacts.download_artifacts(
                artifact_uri=uri, dst_path=destination_path
            )
            LOGGER.info(
