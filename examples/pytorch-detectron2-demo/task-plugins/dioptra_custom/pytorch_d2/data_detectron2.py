...
def register_dataset(dataset_name,
    ...
    if (dataset_type == "detectron2_balloon_json"):
        dataset_dict = lambda d=dataset_path: get_json_dicts(d,
                                                             class_names,
                                                             poison,
                                                             testing,
                                                             poison_path,
        ...
        dataset_dict = lambda d=dataset_path: get_voc_dicts(d,
                                                            class_names,
                                                            poison,
                                                            testing,
                                                            poison_path,
...
def gen_rand_coordinates(width, height, in_width, in_height):
    return (random.randrange(0, width - in_width), random.randrange(0, height - in_height))
...
def get_json_dicts(img_dir, class_names, poison, testing, poison_path, poison_class_label,
                   poison_class_target, poison_scale, poison_color,poison_rel_x_location, poison_rel_y_location):
    ...
    for idx, v in enumerate(imgs_anns.values()):
        ...
        if poison:
            ...
            record = {}
            filename = os.path.join(poison_path, v["filename"].replace('.jpg', '-poisoned.jpg'))
            record["file_name"] = filename
            ...
            for _, anno in annos.items():
                ...
                objs.append(obj)
                image = create_poisoned_image(image.convert('RGBA'), bbox, poison_scale, poison_rel_x_location, poison_rel_y_location,poison_color)
...
def get_voc_dicts(base_path, class_names, poison, testing, poison_path, poison_class_label,
                   poison_class_target, poison_scale, poison_color, poison_rel_x_location, poison_rel_y_location,):
    ...
    for file in filelist:
        ...
        if poison:
            ...
            filename = root.find('filename').text
            new_filename = os.path.join(poison_path, filename.replace('.png', '-poisoned.png'))
            width, height = image.size
            ...
            for boxes in root.iter('object'):
                ...
                if class_index == poison_class_target:
                    image = create_poisoned_image(image.convert('RGBA'), bbox, poison_scale, poison_rel_x_location,
                                                  poison_rel_y_location, poison_color)
                    image = image.convert('RGB')
