...
@click.option(
    "--dataset-tar-name",
    type=click.STRING,
    help="Name of poisoned training tarfile.",
    default="adversarial_poison.tar.gz",
)
@click.option(
    "--dataset-name",
    type=click.STRING,
    help="Name of poisoned training dataset directory.",
    default="adv_poison_data",
)
@click.option(
    "--apply-defense",
    type=click.BOOL,
    help="If set to true, apply PGD defense over model during training.",
    default=False,
)
@click.option(
    "--seed",
...
def train(
    ...
    with mlflow.start_run() as active_run:
        ...
        if load_dataset_from_mlruns:
            ...
            data_train_tar_name = dataset_tar_name
        ...
        if apply_defense:
            ...
            proxy = AdversarialTrainerMadryPGD(
                model, nb_epochs=10, eps=0.15, eps_step=0.001
            )
            ...
            model = proxy.get_classifier().model
            model.compile(
                loss="categorical_crossentropy",
