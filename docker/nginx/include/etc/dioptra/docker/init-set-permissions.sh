...
print_help()
{
  printf '%s\n' "Container Initialization - Set Permissions
"
  printf 'Usage: %s [-r|--(no-)recursive] [--dir-chmod <arg>] [--file-chmod <arg>] [--chown <arg>] [-h|--help] <dest-1> [<dest-2>] ... [<dest-n>] ...\n' "$0"
  printf '\t%s\n' "<dest>: List of filepaths"
  printf '\t%s\n' "-r, --recursive, --no-recursive: Recursive copying, <dest> must be a list of directories (off by default)"
...
{
  local _required_args_string="'dest'"
...
assign_positional_args()
{
  ...
  _positional_names="_arg_dest "
  _our_args=$((${#_positionals[@]} - 1))
  for ((ii = 0; ii < _our_args; ii++))
  do
    _positional_names="$_positional_names _arg_dest[$((ii + 1))]"
  done
