...
GENERATE_PASSWORDS = {{ cookiecutter.generate_passwords.lower() == "true" }}
...
PASSWORD_FILES = [
    ...
    Path("secrets") / "{{ cookiecutter.container_slug_prefix }}-db.env",
    Path("secrets") / "{{ cookiecutter.container_slug_prefix }}-dbadmin.env",
    Path("secrets") / "{{ cookiecutter.container_slug_prefix }}-minio-users.env",
    Path("secrets") / "{{ cookiecutter.container_slug_prefix }}-minio.env",
    Path("secrets") / "{{ cookiecutter.container_slug_prefix }}-mlflow-tracking-database-uri.env",
    Path("secrets") / "{{ cookiecutter.container_slug_prefix }}-mlflow-tracking.env",
    Path("secrets") / "{{ cookiecutter.container_slug_prefix }}-restapi-database-uri.env",
    Path("secrets") / "{{ cookiecutter.container_slug_prefix }}-restapi.env",
    Path("secrets") / "{{ cookiecutter.container_slug_prefix }}-worker.env",
]
...
def insert_random_passwords(password_files, words_file, env):
    ...
    variables = dict(
        ...
        postgres_admin_password = _generate_random_password(words),
...
def render_absolute_path_to_base_directory():
    logger.info("Scanning files and replacing the \"%s\" symbol with %s", BASE_DIRECTORY_SYMBOL, str(BASE_DIRECTORY))
...
def _generate_random_password(
    ...
    end_number = [str(x) for x in random.sample(range(10), k=1)] if append_number else []
...
def _populate_words(words_file, source_encoding="utf-8", unicode_normalize_form="NFKD"):
    ...
    with open(words_file, "rb") as f:
        for line in f:
            ...
            is_not_short: bool = len(normalized_line) >= 5
...
if __name__ == "__main__":
    ...
    if GENERATE_PASSWORDS:
        ...
        logger.info(
            "Skipping password generation, be sure to fill these in yourself!"
        )
