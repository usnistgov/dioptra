        ...
        for artifact_parameter in entrypoint.artifact_parameters:
            try:
                ...
                artifact = self._uow.artifact_repo.get_one_snapshot(
                    value["snapshot_id"], DeletionPolicy.NOT_DELETED
                )
...
class JobIdService(UnitOfWorkService):
    ...
    def get(self, job_id: int, **kwargs) -> utils.JobDict:
        ...
        job = self._uow.job_repo.get(job_id, DeletionPolicy.ANY)

        if not job:
            raise EntityDoesNotExistError(RESOURCE_TYPE, resource_id=job_id)

        elif job.resource.is_deleted:
            raise EntityDoesNotExistError(RESOURCE_TYPE, resource_id=job_id)
    ...
    def get_parameter_values(
        ...
        job = self._uow.job_repo.get_one(
            resource_id=job_id, deletion_policy=DeletionPolicy.NOT_DELETED
        )
...
class ExperimentJobIdService(UnitOfWorkService):
    ...
    def get(self, experiment_id: int, job_id: int, **kwargs) -> utils.JobDict:
        ...
        # TODO: is this good practice, or should the job repository have a separate
        #       getter that handles checking the experiment?
        if (
            job.experiment_job.experiment_id != experiment_id
            or job.experiment_job.experiment.resource.is_deleted
        ):
            raise EntityDoesNotExistError(RESOURCE_TYPE, resource_id=job_id)
...
class JobLogService(object):
    ...
    def get_logs(
        ...
        **kwargs,
        ...
        logs, total_num_logs = self._uow.job_repo.get_logs(
            job_id,
            search_struct,
            page_index,
            page_length,
