...
class EntrypointRepository:
    ...
    def create(self, entrypoint: EntryPoint):
        ...
        Raises:
            EntityExistsError: if the entrypoint resource or snapshot already
                ...
                in the same group
            EntityDoesNotExistError: if any child entrypoint doesn't exist, or
        ...
        utils.assert_can_create_resource(
            self.session, entrypoint, EntityType.ENTRY_POINT
        )
    ...
    def create_snapshot(self, entrypoint: EntryPoint):
        ...
        Raises:
            EntityDoesNotExistError: if the entrypoint resource, snapshot
                creator user, or any child entrypoint does not exist
    ...
    def get_exact(
        ...
        """
        Get the exact list of snapshots for the given entrypoint resource IDs.

        Use this when a caller requires that every ID in the resource_ids sequence
        returns an EntryPoint object.

        Args:
            resource_ids: An or iterable of entrypoint resource IDs
            deletion_policy: Whether to look at deleted entrypoints,
                non-deleted entrypoints, or all entrypoints
        ...
        Raises:
            EntityDoesNotExistError: if any of the entrypoints do not exist in the
    ...
    def get_one_snapshot(
        ...
        """
        Get the specific entrypoint snapshot given the resource snapshot ID; require
    ...
    def create_queues(
        ...
        """
        Add the given queues as children of the given entrypoint.

        Args:
            entrypoint: An EntryPoint object
            queues: The queues to add as children
    ...
    def set_queues(
        ...
        Raises:
            EntityDoesNotExistError: if entrypoint or any queue not exist
            EntityDeletedError: if entrypoint or any queue is deleted
        """
    ...
    def unlink_queue(
        ...
        """
        "Unlink" the given queue resource from the given entrypoint.  This
        ...
        Args:
            ...
            queue: A queue or resource_id integer primary key value

        Raises:
            EntityDoesNotExistError: if parent or child do not exist
        """
        utils.unlink_child(self.session, entrypoint, queue, EntityType.QUEUE)
    ...
    def unlink_queues(
        ...
        """
        "Unlink" all queue child resources from the given entrypoint.  This
        only severs the relationship; it does not delete either resource.  If
        there is no parent/child relationship, this is a no-op.
