...
class DraftsRepository:
    ...
    def create_draft_modification(
        ...
        result = self._session.scalar(sa.select(stmt.exists()))
        if not result:
    ...
    def update(
        self, draft: DraftResource | int, payload: dict[str, Any]
    ) -> DraftResource:
        """
        Replace the payload of the given draft with the given payload.
        ...
        Returns:
            An updated DraftResource object
        ...
        if isinstance(draft, int):
            temp = self.get(draft)
            if not temp:
                # but it should exist here since we just checked it above!
                raise DraftDoesNotExistError(draft_resource_id=draft)
            draft = temp

        # TODO: verify that the given payload is of the same type (resource
        # vs modification) as the previous payload?
