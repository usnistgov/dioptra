...
class ArtifactRepository:
    ...
    def create(self, artifact: Artifact) -> None:
        ...
        # Consistency rules:
        # - Artifact uris must be globally unique
        ...
        self._assert_uri_unique(artifact)
    ...
    def _assert_uri_unique(self, artifact: Artifact) -> None:
        """Assert that no other artifact has ever used this URI."""
        stmt = select(Artifact).where(Artifact.uri == artifact.uri)

        if artifact.resource_id is not None:
            stmt = stmt.where(Artifact.resource_id != artifact.resource_id)
