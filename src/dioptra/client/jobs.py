...
class JobsCollectionClient(CollectionClient[T]):
    ...
    def snapshots(self) -> SnapshotsSubCollectionClient[T]:
        ...
        that own the existing job snapshots sub-collection. Below are examples of
        ...
        active Python Dioptra Python client called ``client``::
            ...
            client.jobs.existing_drafts.get_by_id(1)

            # GET /api/v1/jobs/1/snapshots/2
            client.jobs.existing_drafts.get_by_id(1, snapshot_id=2)
        """
    ...
    def get(
        ...
        Args:
            group_id: The group ID the jobs belong to. If None, return
                jobs from all groups that the user has access to.
