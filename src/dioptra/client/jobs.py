...
class JobsCollectionClient(CollectionClient[T]):
    ...
    def get_logs_by_id(
        ...
        """
        Get log records for the given job resource.  Records are returned in
    ...
    def append_logs_by_id(self, job_id: str | int, logs: Iterable[dict[str, Any]]) -> T:
        ...
        Legal values for severity include DEBUG, INFO, WARNING, ERROR,
        CRITICAL.  The step name is intended to capture which step of an
        experiment was being executed at the time the log record was emitted.
        Step names and timestamps are optional.  Timestamps, if given, must be
        ISO strings.  If a timestamp is not included, the server's current
