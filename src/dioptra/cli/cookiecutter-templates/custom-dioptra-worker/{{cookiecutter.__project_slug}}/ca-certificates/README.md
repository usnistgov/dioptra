...
Most users do not need to add anything to this folder. If you are unsure whether you need custom CA certificates, try building the image first without them. If the build fails with an HTTPS or SSL error, that is a sign you need to add certificates here.
...
4. Build the Docker image as usual with `docker build`.
