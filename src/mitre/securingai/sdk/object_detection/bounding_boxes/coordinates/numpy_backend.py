...
class NumpyBoundingBoxCoordinates(BoundingBoxCoordinates):
    ...
    def apply_constraint_one_object_per_cell(
        self,
        bboxes_cell_xywh: np.ndarray,
        bboxes_cell_ij: np.ndarray,
        bboxes_labels: np.ndarray,
    ...
    def find_bbox_cell_ij(
        ...
        return np.stack([i, j], axis=-1).astype("int32")

    def find_bbox_cell_xy(
        self, x_center: np.ndarray, y_center: np.ndarray, bboxes_cell_ij: np.ndarray
    ) -> np.ndarray:
        cell_h: float = self.cell_height
        cell_w: float = self.cell_width
    ...
    def from_corner_to_image_xywh(self, bboxes_corner: np.ndarray) -> np.ndarray:
    ...
    def from_image_to_cell_xywh(
        self, bboxes_image_xywh: np.ndarray
    ...
    def from_corner_to_cell_xywh(
        self, bboxes_corner: np.ndarray
    ) -> Tuple[np.ndarray, np.ndarray]:
        bboxes_image_xywh: np.ndarray = self.from_corner_to_image_xywh(
    ...
    def from_cell_xywh_to_image_xywh(
        self, bboxes_cell_xywh: np.ndarray, bboxes_cell_ij: np.ndarray
    ) -> np.ndarray:
    ...
    def from_image_xywh_to_corner(self, bboxes_image_xywh: np.ndarray) -> np.ndarray:
...
class NumpyBoundingBoxesBatchedGrid(BoundingBoxesBatchedGrid):
    ...
    def embed(
        self, bboxes_corner: np.ndarray, bboxes_labels: np.ndarray, n_classes: int
    ) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        bboxes_cell_xywh_grid: np.ndarray = np.zeros(
            shape=(self.cell_nrow, self.cell_ncol, 4),
            dtype="float32",
        )
        bboxes_labels_grid: np.ndarray = np.zeros(
            shape=(self.cell_nrow, self.cell_ncol, n_classes),
            dtype="float32",
        )
        bboxes_object_mask: np.ndarray = np.zeros(
            shape=(self.cell_nrow, self.cell_ncol),
            dtype="float32",
        )
        bboxes_no_object_mask: np.ndarray = np.ones(
            shape=(self.cell_nrow, self.cell_ncol),
            dtype="float32",
        )
    ...
    def from_image_to_cell_xywh(self, bboxes_image_xywh: np.ndarray) -> np.ndarray:
    ...
    def from_corner_to_cell_xywh(self, bboxes_corner: np.ndarray) -> np.ndarray:
        bboxes_image_xywh: np.ndarray = self.from_corner_to_image_xywh(
            bboxes_corner=bboxes_corner
        )
        bboxes_cell_xywh: np.ndarray = self.from_image_to_cell_xywh(
            bboxes_image_xywh=bboxes_image_xywh
        )
    ...
    def from_cell_xywh_to_image_xywh(
        self, bboxes_cell_xywh: np.ndarray, n_bounding_boxes: int
    ) -> np.ndarray:
    ...
    @staticmethod
    def _generate_wh_grid_indices(grid_shape: Tuple[int, int]) -> np.ndarray:
        ...
        return np.concatenate([w_grid_indices, h_grid_indices], axis=-1).astype(
            "float32"
        )

    @staticmethod
    def _generate_num_wh_grid_indices(grid_shape: Tuple[int, int]) -> np.ndarray:
        ...
        return np.concatenate([num_w_grid_indices, num_h_grid_indices], axis=-1).astype(
            "float32"
        )
