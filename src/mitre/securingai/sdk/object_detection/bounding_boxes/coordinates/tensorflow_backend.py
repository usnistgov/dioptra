...
class TensorflowBoundingBoxCoordinates(BoundingBoxCoordinates):
    ...
    def _find_no_obj_cell_ij(
        self, bboxes_cell_ij: np.ndarray, i_range: np.ndarray, j_range: np.ndarray
    ) -> np.ndarray:
        cell_ij_set = set([(i, j) for i in i_range for j in j_range])
...
class TensorflowBoundingBoxesBatchedGrid(BoundingBoxesBatchedGrid):
    ...
    def _embed(
        ...
        ij: np.ndarray,
        labels: np.ndarray,
        bboxes_cell_xywh_grid: np.ndarray,
        bboxes_labels_grid: np.ndarray,
        bboxes_object_mask: np.ndarray,
        bboxes_no_object_mask: np.ndarray,
