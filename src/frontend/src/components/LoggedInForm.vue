<template>
  ...
  >
    <q-card-section class="text-center">
      ...
      <p>
        You are currently logged in as <span class="text-weight-bold text-primary">{{ loggedInUser.username }}</span>
      </p>
      <p>
        Email: {{ loggedInUser.email }} <br />
        Password Expires:
        {{
          new Date(loggedInUser.passwordExpiresOn).toLocaleString("en-US", {
            year: "numeric",
            month: "long",
            day: "numeric",
          })
    ...
    <p class="text-body1 text-bold q-mt-xl">Additional User Account Actions</p>
    ...
    >
      <q-form @submit="callChangePassword()">
        <q-card-section class="text-center shadow-2 q-mt-sm q-mb-sm">
          <p>To change your password, enter your current and new password.</p>
          <q-input
            v-model="password"
            class="q-mb-sm"
            outlined
    ...
    >
      <q-form @submit="callDeleteUser()">
        <q-card-section class="text-center shadow-2 q-mt-sm q-mb-sm">
          <p>To delete your account, enter your password and press the delete button.</p>
          <q-input
