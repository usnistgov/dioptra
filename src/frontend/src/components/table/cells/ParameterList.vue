  ...
  <div class="column q-gutter-y-xs">
    ...
    <template v-for="(param, i) in items.slice(0, 3)" :key="i">
      <div class="row no-wrap q-gutter-x-sm">
        ...
        <div class="column items-baseline no-wrap q-gutter-x-sm">
          ...
          <div class="row">
            <div class="font-mono text-weight-medium text-grey-7 cursor-help q-pr-xs" style="font-size: 0.75rem">
            ...
            <div v-if="!param.required & concept=='input'" class="text-weight-medium text-grey-5" style="font-size: 0.7rem">
                (optional)
            </div>
        </div>
        </div>
    ...
    <div v-if="items.length > 3" class="row no-wrap q-gutter-x-md">
      ...
      >
        ...
        <q-menu anchor="bottom middle" self="top left" class="bg-white shadow-5 border-grey-3">
          <div class="column q-pa-sm q-gutter-y-xs" >
            <div class="text-caption text-grey-7 q-mb-xs">Additional {{ type === 'input' ? 'Inputs' : 'Outputs' }}</div>
            ...
            <template v-for="(param, i) in items.slice(3)" :key="i">
              <div class="row items-center no-wrap q-gutter-x-sm q-py-xs">
                ...
                <div class="row items-baseline no-wrap q-gutter-x-sm">
                  ...
                  <div class="font-mono text-weight-medium text-grey-7" style="font-size: 0.75rem">
    ...
    <div v-if="items.length === 0" class="text-caption text-grey-4">
      -
    </div>
...
const props = defineProps({
  ...
  type: { type: String, default: 'output' } // 'input' or 'output'
})

// Compute Colors based on Concept Type
const styles = computed(() => {
  // Map 'input'/'output' strings to concept styles if they exist, 
  // or fallback to hardcoded defaults mimicking the request
  const concept = props.type === 'input' ? 'input' : 'output'
  const styleObj = getConceptStyle(concept)
  
  // Defaults if getConceptStyle doesn't return specific colors for input/output
  ...
  return {
    ...
    textColor: getPaletteColor(fallbackText) // Or use styleObj.textColor if you add that
  }
})
