<template>
  <TableComponent
    ...
    :preserveSort="false"
  >
    <template #body-cell-snapshotCreatedOn="cellProps">
      <div :data-snapshot-id="cellProps.row.snapshot">
        {{
          new Intl.DateTimeFormat("en-US", {
            ...
            minute: "numeric",
            hour12: true,
          }).format(new Date(cellProps.row.snapshotCreatedOn))
        }}
        <q-chip
          v-if="cellProps.row.latestSnapshot"
          label="latest"
          size="md"
          dense
          color="orange"
  ...
  try {
    ...
    if (res.data.data.length > 0) {
      if (route.query.snapshotId) {
        ...
        const index = res.data.data.findIndex((obj) => obj.snapshot === Number(route.query.snapshotId));
      ...
      await nextTick();
      const el = document.querySelector(`[data-snapshot-id="${selected.value[0]?.snapshot}"]`);
...
watch(selected, (newVal) => {
  if (newVal.length > 0) {
    ...
    history.replaceState({}, null, route.path + "?snapshotId=" + encodeURIComponent(selected.value[0].snapshot));
  }
});
