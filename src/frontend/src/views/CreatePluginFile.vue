<template>
  ...
  <div class="row q-my-lg">
    ...
    <fieldset :class="`${isMobile ? 'col-12' : 'col q-ml-md'}`">
      ...
      >
        ...
        <template #body-cell-inputParams="cellProps">
          ...
          <q-btn
            ...
            @click="
              handleSelectedParam('create', cellProps, i, 'inputParams', 'functions');
              showEditParamDialog = true;
            "
          />
        </template>
        <template #body-cell-outputParams="cellProps">
          ...
          <q-btn
            ...
            class="q-mr-xs q-my-xs"
            @click="
              handleSelectedParam('create', cellProps, i, 'outputParams', 'functions');
              showEditParamDialog = true;
            "
          />
        </template>
      ...
      >
        ...
        >
          <template #body-cell-view="cellProps">
            <q-btn
              label="View"
              color="primary"
              @click.stop="
                structure = JSON.stringify(cellProps.row.structure, null, 2);
  ...
  <div :class="`${isMobile ? '' : ''} float-right q-mb-lg`">
    <q-btn
      ...
      @click="
        confirmLeave = true;
        store.initialPage ? router.push(`/plugins/${route.params.id}`) : router.back();
      "
    />
    <q-btn
      color="primary"
  ...
  <DeleteDialog
    ...
    @submit="
      pluginFile.tasks[selectedTaskProps.taskType].splice(selectedTaskProps.rowIndex, 1);
...
async function submit() {
  basicInfoForm.value.validate().then((success) => {
    ...
    const missingTypes = pluginFile.value.tasks.functions.some((task) =>
      [...task.inputParams, ...task.outputParams].some((param) => !param.parameterType.id),
    );
    if (missingTypes) {
...
function updateParam(updatedParam) {
  pluginFile.value.tasks[selectedTaskProps.value.functionsOrArtifacts][selectedTaskProps.value.rowIndex][
    selectedTaskProps.value.inputOrOutputParams
  ][selectedTaskProps.value.paramIndex] = updatedParam;
}
