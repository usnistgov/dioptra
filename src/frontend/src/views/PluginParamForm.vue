<template>
  ...
  <div :class="`row q-my-lg ${isMobile ? '' : 'q-gutter-x-xl'}`">
    <fieldset
      ...
      :disabled="pluginParamType.deleted"
    >
  ...
  <div class="float-right">
    <q-btn
      ...
      @click="
        confirmLeave = true;
        store.initialPage ? router.push(`/pluginParams`) : router.back();
      "
    />
    <q-btn
      color="primary"
      ...
      @click="submit()"
    >
      <q-tooltip v-if="jsonError !== ''"> Fix invalid JSON </q-tooltip>
      <q-tooltip v-else-if="!valuesChangedFromEditStart"> No changes detected — nothing to save </q-tooltip>
    </q-btn>
  </div>
