  ...
  >
    <template #body-cell-hasDraft="cellProps">
      <q-btn
        ...
        round
        size="sm"
        :icon="cellProps.row.hasDraft ? 'edit' : 'add'"
        :color="cellProps.row.hasDraft ? 'primary' : 'grey-5'"
        @click.stop="router.push(`/queues/${cellProps.row.id}/resourceDraft/${cellProps.row.hasDraft ? '' : 'new'}`)"
      />
    </template>
  </TableComponent>
