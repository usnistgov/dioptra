<template>
  ...
  >
    <template #header>
      ...
      <q-item-section>
        <q-item-label> {{ showMetadata ? "Hide" : "Show" }} "{{ ORIGINAL_PLUGIN.name }}" Metadata </q-item-label>
        <q-item-label
  ...
  >
    <template #body-cell-functionTasks="cellProps">
      {{ cellProps.row.tasks.functions.length }}
    </template>
    <template #body-cell-artifactTasks="cellProps">
      {{ cellProps.row.tasks.artifacts.length }}
    </template>
  </TableComponent>
