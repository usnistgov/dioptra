...
export async function addJobMetric(id: string, name: string, value: number, step: number) {
  await axios.post(`/api/jobs/${id}/metrics`, {
    name,
    value,
    step,
  });
...
export async function appendResource<T extends ResourceType>(
  parentResourceType: T,
...
export async function removeResourceFromResource<T extends ResourceType>(
  parentResourceType: T,
  parentResourceId: number,
  childResourceType: T,
  id: number,
