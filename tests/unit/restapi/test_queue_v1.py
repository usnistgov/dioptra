...
def register_queue(
    client: FlaskClient, name: str, description: str = "", group_id: str = ""
) -> TestResponse:
...
# TODO by names not supported in v1
...
#     """
#     return client.delete(
#         f"/{V1_ROOT}/{QUEUE_ROUTE}/name/{name}",
#         follow_redirects=True,
#     )

# TODO when locks are implemented
...
#     """
#     return client.put(
#         f"/{V1_ROOT}/{QUEUE_ROUTE}/{queue_id}/lock",
#         follow_redirects=True,
#     )
...
#     """
#     return client.put(
#         f"/{V1_ROOT}/{QUEUE_ROUTE}/name/{name}/lock",
#         follow_redirects=True,
#     )
...
#     """
#     response = client.get(
#         f"/{V1_ROOT}/{QUEUE_ROUTE}/name/{queue_name}", follow_redirects=True
#     )
#     assert response.status_code == 200 and response.get_json() == expected
...
def assert_retrieving_all_queues_works(
    ...
    Raises:
        AssertionError: If the response status code is not 200 or if the API response
            does not match the expected response.
    """
    response = client.get(f"/{V1_ROOT}/{QUEUE_ROUTE}", follow_redirects=True)
...
def test_queue_name_is_slugified(client: FlaskClient, db: SQLAlchemy) -> None:
    ...
    queue_id = response.get_json()['data'][0]["queueId"]
...
def test_get_queue_by_query(client: FlaskClient, db: SQLAlchemy) -> None:
    queue1_response = register_queue(
        client, name="tensorflow_cpu", description="string1", group_id=1
    )
    queue2_response = register_queue(
        client, name="pytorch_cpu", description="string2", group_id=1
    )
    queue1_expected = queue1_response.get_json()
    queue2_expected = queue2_response.get_json()
    queue_expected_list = [queue1_expected, queue2_expected]
    ...
    queue3_response = register_queue(
        client, name="pytorch_gpu", description="The third queue.", group_id=1
    )
...
# TODO by name not supported in V1
