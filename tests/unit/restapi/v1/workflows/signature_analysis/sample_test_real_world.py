...
from .artifacts_restapi import (
    get_uri_for_model,
    get_uris_for_artifacts,
    get_uris_for_job,
)
from .artifacts_utils import extract_tarfile, make_directories
from .attacks_fgm import fgm
from .attacks_patch import create_adversarial_patch_dataset, create_adversarial_patches
from .backend_configs_tensorflow import init_tensorflow
from .data_tensorflow import (
...
from .metrics_performance import evaluate_metrics_generic, get_performance_metric_list
...
from .tracking_mlflow import log_metrics, log_parameters, log_tensorflow_keras_estimator
...
def load_dataset(
    ...
    training_dataset = (
        None
        if "training" not in subsets
        else create_image_dataset(
...
def create_model(
    ...
    classifier = init_classifier(
        model_architecture, optim, perf_metrics, input_shape, n_classes, loss
    )
    return classifier
...
def load_model(
    ...
    if art:
        classifier = load_wrapped_tensorflow_keras_classifier(
            uri, imagenet_preprocessing, image_size, classifier_kwargs
        )
    else:
...
def save_artifacts_and_models(
    artifacts: List[Dict[str, Any]] = None, models: List[Dict[str, Any]] = None
):
...
def load_artifacts_for_job(
    job_id: str, files: List[str | Path] = None, extract_files: List[str | Path] = None
):
...
@pyplugs.register
def model_metrics(classifier: Any, dataset: Any):
...
def predict(
    ...
    df = predictions_to_df(
        predictions, dataset, show_actual=show_actual, show_target=show_target
    )
    return df
