...
def assert_retrieving_all_tags_works(
    client: FlaskClient,
...
def test_tag_search_query(
    ...
    - In all cases, the returned information matches the information that was provided
      during registration.
    """
    tag_expected_list = list(registered_tags.values())[:2]
...
def test_cannot_register_existing_tag_name(
    ...
    assert_registering_existing_tag_name_fails(
        ...
        group_id=existing_tag["group"]["id"],
    )
...
def test_rename_tag(
    ...
    - The user is able to retrieve information about the "ML: Object Recognition" tag that
...
def test_delete_tag_by_id(
    ...
    - The user attempts to retrieve information about the tag, which
      is no longer found.
    """
    tag_to_delete = registered_tags["tag1"]
    delete_tag(client, tag_id=tag_to_delete["id"])
    assert_tag_is_not_found(client, tag_id=tag_to_delete["id"])
