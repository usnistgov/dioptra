...
def assert_sorting_artifact_by_none_works(
    dioptra_client: DioptraClient[DioptraResponseProtocol],
    expected: set[str],
    sort_by: str | None,
    descending: bool | None,
    ...
    """Assert that artifacts can be retrieved properly when sorting isn't specified.

    Args:
        client: The Flask test client.
        expected: The expected artifact ids.
    ...
    response_data = response.json()
    artifact_ids = [artifact["id"] for artifact in response_data["data"]]
    assert response.status_code == HTTPStatus.OK and set(artifact_ids) == expected
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (None, None, ["artifact3", "artifact2", "artifact1", "artifact4"]),
    ],
)
def test_artifact_sort_by_none(
    dioptra_client: DioptraClient[DioptraResponseProtocol],
    db: SQLAlchemy,
    auth_account: dict[str, Any],
    registered_artifacts: dict[str, Any],
    ...
    """Test that artifacts can be retrieved when sorting is unspecified.
    ...
    - A user registers four artifacts with these descriptions:
        ...
        "Fine-tuned model.".
    - The user is able to retrieve a list of all registered artifacts.
    - The returned list of artifacts matches regardless of the order in
        the parametrize lists above.
    """
    expected_ids = set(
        [registered_artifacts[expected_name]["id"] for expected_name in expected]
    )
    assert_sorting_artifact_by_none_works(
