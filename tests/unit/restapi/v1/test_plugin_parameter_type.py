...
def assert_sorting_plugin_parameter_type_by_none_works(
    dioptra_client: DioptraClient[DioptraResponseProtocol],
    expected: set[str],
    sort_by: str | None,
    descending: bool | None,
    ...
    """Assert that plugin parameter types can be retrieved properly when sorting
        isn't specified.

    Args:
        client: The Flask test client.
        expected: The expected type ids.
          See test_plugin_parameter_type_sort for expected orders.
    ...
    assert response.status_code == HTTPStatus.OK and set(param_ids) == expected
...
def test_plugin_parameter_type_sort_by_none(
    dioptra_client: DioptraClient[DioptraResponseProtocol],
    db: SQLAlchemy,
    auth_account: dict[str, Any],
    registered_plugin_parameter_types: dict[str, Any],
    ...
    """Test that plugin param types can be retrieved when sorting is unspecified.

    Given an authenticated user and registered types, this test validates the following
    sequence of actions:

    - A user registers three types, "image_shape", "model_output", "model".
    - The user is able to retrieve a list of all registered types.
    - The returned list of plugin param types matches regardless of the order in the
    ...
    assert_sorting_plugin_parameter_type_by_none_works(
        dioptra_client, sort_by=sort_by, descending=descending, expected=expected_ids
    )
