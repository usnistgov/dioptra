...
def make_job_snapshot_peer(source_job, creator):
    """Create a distinct job using the same experiment, entrypoint, and queue."""
    job_resource = models.Resource("job", source_job.resource.owner)
    job = models.Job(
        timeout=source_job.timeout,
        status=source_job.status,
        description="peer job",
        resource=job_resource,
        creator=creator,
    )
    job.entry_point_job = models.EntryPointJob(
        job_resource=job_resource,
        entry_point=source_job.entry_point_job.entry_point,
        entry_point_parameter_values=[],
        entry_point_artifact_parameter_values=[],
    )
    job.experiment_job = models.ExperimentJob(
        job_resource=job_resource,
        experiment=source_job.experiment_job.experiment,
    )
    job.queue_job = models.QueueJob(
        job_resource=job_resource,
        queue=source_job.queue_job.queue,
    )
    return job
...
class TestArtifactCreate:
    ...
    def test_artifact_create_duplicate_uri_across_jobs(
        self, db_session: DBSession, account, artifact_repo, job_with_dependencies
    ):
        uri = "s3://test-bucket/globally-unique-artifact"
        artifact = make_artifact(
            account.user,
            account.group,
            job_with_dependencies,
        ...
        peer_job = make_job_snapshot_peer(job_with_dependencies, account.user)
        db_session.add(peer_job)
        db_session.commit()
        ...
        with pytest.raises(errors.EntityExistsError):
            artifact_repo.create(duplicate)
...
class TestArtifactGet:
    ...
    def test_artifact_get_multiple(
        ...
        artifact2 = make_artifact(
            account.user,
            account.group,
            job_with_dependencies,
            uri="s3://test-bucket/artifact2",
        )
        artifact_repo.create(artifact2)
        db_session.commit()

        result = artifact_repo.get(
            [artifact1.resource_id, artifact2.resource_id],
            utils.DeletionPolicy.NOT_DELETED,
        )
        assert len(result) == 2
        assert {artifact.resource_id for artifact in result} == {
            artifact1.resource_id,
            artifact2.resource_id,
        }
