...
def rename_experiment_with_id(
    ...
    """
    return client.put(
        f"/api/{EXPERIMENT_BASE_ROUTE}/{id}",
        json={"name": new_name},
        follow_redirects=True,
    )
    
def delete_experiment_with_name(
    client: FlaskClient, name: str
) -> TestResponse:
...
def assert_retrieving_experiment_by_id_works(
    ...
    Raises:
        AssertionError: If the response status code is not 200 or if the API response
            does not match the expected response.
    """
    response = client.get(f"/api/{EXPERIMENT_BASE_ROUTE}/{id}", follow_redirects=True)
    assert response.status_code == 200 and response.get_json() == expected
...
def test_register_experiment(
    ...
    """Tests that experiments can be registered through the api following the scenario 
       below.
    ...
    assert_retrieving_experiment_by_name_works(
        client, queue_name=experiment1_expected["name"], expected=experiment1_expected
    )
    assert_retrieving_experiment_by_id_works(
        client, id=experiment1_expected["experimentId"], expected=experiment1_expected
    )
    assert_retrieving_experiment_by_name_works(
        client, queue_name=experiment2_expected["name"], expected=experiment2_expected
    )
    assert_retrieving_experiment_by_id_works(
...
def test_cannot_register_existing_experiment_name(
    ...
    """
    register_experiment(client, name="mnist")
    assert_registering_existing_experiment_name_fails(client, name="mnist")


def test_retrieve_experiment(
    client: FlaskClient, db: SQLAlchemy
) -> None:
    """Test that an experiment can be retrieved through the api following the 
       scenario below.
    ...
    - In all cases, the returned information matches the information that was provided
      during registration.
    """
    experiment_expected = register_experiment(client, name="mnist").get_json()
    assert_retrieving_experiment_by_name_works(
        client, queue_name=experiment_expected["name"], expected=experiment_expected
    )
    assert_retrieving_experiment_by_id_works(
...
def test_list_experiments(
    ...
    """Test that the list of experiments can be renamed through the api following the 
       scenario below.
    ...
    experiment_expected_list = [
        experiment1_expected, experiment2_expected, experiment3_expected
    ]
...
def test_rename_experiment(
    ...
    """Test that an experiment can be renamed through the api following the scenario 
       below.
    ...
    rename_experiment_with_id(client, id=experiment_json["experimentId"], new_name=update2_name)
    assert_experiment_name_matches_expected_name(
...
def test_delete_experiment(
    ...
    """Test that a queue can be deleted through the api following the scenario 
       below.
    ...
    - The user is able to retrieve information about the "mnist" experiment that
      matches the information that was provided during registration.
    - The user deletes the "mnist" queue by referencing its name.
    - The user attempts to retrieve information about the "mnist" experiment, which
      is no longer found.
    ID:
    - A user registers another experiment named "imagenet".
    - The user is able to retrieve information about the "imagenet" experiment that
      matches the information that was provided during registration.
    - The user deletes the "imagenet" experiment by referencing its id.
    - The user attempts to retrieve information about the "imagenet" experiment, which
      is no longer found.
    """
    name_by_name = "mnist"
    experiment1_json = register_experiment(client, name=name_by_name).get_json()
    assert_retrieving_experiment_by_id_works(
        client, queue_id=experiment1_json["experimentId"], expected=experiment1_json
    )
    delete_experiment_with_name(client, name=name_by_name)
    assert_experiment_is_not_found(
        client, experiment_id=experiment1_json["experimentId"]
    )
    
    name_by_id = "imagenet"
    experiment2_json = register_experiment(client, name=name_by_id).get_json()
    assert_retrieving_experiment_by_id_works(
        client, queue_id=experiment2_json["experimentId"], expected=experiment2_json
    )
    delete_experiment_with_id(client, id=experiment2_json["experimentId"])
    assert_experiment_is_not_found(
        client, experiment_id=experiment2_json["experimentId"]
    )    
