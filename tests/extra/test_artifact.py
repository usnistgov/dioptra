...
@pytest.mark.parametrize(
    "name, contents, type, suffix", [("artifact1", "file1", "txt", ".txt")]
)
def test_string_task(name: str, contents: str, type: str, suffix: str):
    work_dir = Path.cwd() / "tmp"
    create_artifact(Path.cwd(), DirTest("tmp"))
    result = StringArtifactTask.serialize(work_dir, name, contents, type)
    assert result == (work_dir / name).with_suffix(suffix)
...
@pytest.mark.parametrize(
    "name, contents, type, suffix",
    [
        ("artifact1", "file1".encode(), "", ""),
        ("mypdf", "some sort of pdf endcoding".encode(), "pdf", ".pdf"),
    ],
)
def test_bytes_task(name: str, contents: bytes, type: str, suffix: str):
    work_dir = Path.cwd() / "tmp"
    create_artifact(Path.cwd(), DirTest("tmp"))
    result = BytesArtifactTask.serialize(work_dir, name, contents, type)
    assert result == (work_dir / name).with_suffix(suffix)
    assert result.is_file()
...
def test_file_task(name: str, contents: str):
    work_dir = Path.cwd() / "tmp"
    create_artifact(Path.cwd(), DirTest("tmp", [FileTest(contents)]))
    result = FileArtifactTask.serialize(work_dir, name, contents)
...
def test_file_task_fail(name: str, contents: str):
    """Test to verify exception is thrown if file does not exist"""
    work_dir = Path.cwd() / "tmp"
    with pytest.raises(FileArtifactError):
        FileArtifactTask.serialize(work_dir, name, contents)
...
def test_directory_task(name: str, ext: str, dir: DirTest):
    work_dir = Path.cwd() / "tmp"
    create_artifact(work_dir, dir)
    result = DirectoryArtifactTask.serialize(work_dir, name, dir.name)

    assert result == (work_dir / f"{name}.{ext}")
    extracted = DirectoryArtifactTask.deserialize(
...
def test_dataframe_task(name: str, data: dict, format: str, ext: str):
    df = pd.DataFrame(data=data)

    work_dir = Path.cwd() / "tmp"
    work_dir.mkdir(exist_ok=True)
    result = DataframeArtifactTask.serialize(work_dir, name, df, format=format)
    assert result == (work_dir / f"{name}.{ext}")
    assert result.is_file()

    des_df = DataframeArtifactTask.deserialize(work_dir, f"{name}.{ext}")
