...
.. code:: text
   ...
   datasets_directory []: /path/to/dioptra/datasets
...
.. code:: sh
   ...
   cruft update --variables-to-update '{ "datasets_directory": "/path/to/dioptra/datasets" }'
...
.. code:: text
   ...
   │   │   ├── docker-compose.yml.j2                            <- A Jinja2 template used to generate the docker-compose.yml, which orchestrates how to start the containers, configure the network, set the environment variables, attach the storage volumes, and publish the NGINX web server ports.
   ...
   │   ├── generate_docker_templates.py                         <- Used in the init-deployment.sh script to configure dataset volumes for worker containers.
...
In addition to the above, you may want to further customize the the Docker Compose configuration via the ``docker-compose.override.yml`` file to suit your needs, such as allocating explicit CPUs you want each container to use. An example template file (``docker-compose.override.yml.template``) is provided as part of the deployment as a starting point. This can be compied to ``docker-compose.override.yml`` and modified.
...
A more advanced configuration with can be achieved via the ``docker-compose.override.yml`` file.
...
For this example, let's assume the datasets are stored on the host machine under the absolute path ``/home/datasets`` and that we want to make these available in the worker containers under the absolute path ``/dioptra/data``.
...
Append the line ``- /home/datasets:/dioptra/data:ro`` to the ``volumes:`` subsections.

.. code:: yaml

   dioptra-deployment-tfcpu-01:
     volumes:
       - /home/datasets:/dioptra/data:ro
...
For this example, let's assume the datasets are stored on a NFS version 4 server with IP address 10.0.0.10 and in the exported directory ``/dioptra/datasets`` and that we want to make these available in the worker containers under the absolute path ``/dioptra/data``.
...
The worker container blocks will have **tfcpu**, **tfgpu**, **pytorch-cpu**, or **pytorch-gpu** in their names.
Append the line ``- dioptra-datasets:/dioptra/data:ro`` to the ``volumes:`` subsection.
...
.. code:: yaml
   ...
   dioptra-deployment-tfcpu-01:
     volumes:
       ...
       - dioptra-datasets:/dioptra/data:ro
